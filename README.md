...
## Linux / macOS + 가상환경(venv) 실행 가이드

아래는 bash/zsh 기준. (Python 3.11/3.12 권장)

### 1) 저장소 준비

```bash
cd ~/work
git clone git@github.com:Asakhan/multi_step_qa_langchain.git    # SSH
# 또는: git clone https://github.com/Asakhan/multi_step_qa_langchain.git
cd multi_step_qa_langchain
```
...
활성화되면 프롬프트 앞에 `(.venv)` 가 붙는다. 끝낼 때는 `deactivate`.
...
> 의존성은 **LangChain 0.3.x / OpenAI SDK 1.x** 라인에 고정돼 있다(`requirements.txt` 상단 주석의
> 버전 불변식 참고). 검증된 환경을 **그대로** 재현하려면 동결본을 쓴다:
>
> ```bash
> pip install -r requirements-lock.txt
> ```
...
```bash
cp .env.example .env
${EDITOR:-nano} .env      # OPENAI_API_KEY=sk-... 저장
```
...
```bash
cp -r ~/path/to/kdart_qa/data/index ./data/index
```

**(대안) 청크만 복사 후 재빌드** — 약 $0.03 재임베딩:

```bash
cp -r ~/path/to/kdart_qa/data/chunks ./data/chunks
python -m msqa.build_index --reset
```

> FinQA 는 문항이 자체 컨텍스트를 들고 있어 인덱스가 필요 없다(`data/finqa.jsonl` 포함).
> 정확히 동일한 FinQA 20개 id 로 다시 만들려면:
> `python -m msqa.prepare_finqa --finqa-test ~/path/to/FinQA/dataset/test.json --ids id1,id2,...`
